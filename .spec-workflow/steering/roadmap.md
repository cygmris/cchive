...
- `[~]` **S8 collection-and-mcp** (NEXT — the shared Card/Table/Master-detail collection + MCP manager; produces the MCP count Overview needs) — the shared Card/Table/Master‑detail collection component + MCP manager: read/write `mcpServers` (global `~/.claude.json` + project `.mcp.json`), enable/disable, add/edit/remove, `.mcp.json` preview. Covers 45–51.
