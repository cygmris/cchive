...
- `[~]` **S3 claude-data-core** (next) — `core/paths`, `core/atomic_fs` (temp+fsync+rename, 0600, backup, rollback), `core/credentials` (per‑OS active credential incl. mac Keychain), `core/keyring_store` (account vault), `core/claude_json`, `core/settings`; the typed IPC layer + `model.rs`. Heavy unit tests on switch/atomic/rollback. No new screen — backbone for S4+. (Gotchas G1–G12.)
