//! Rotating, restorable snapshots of the Claude files cchive mutates.
//!
//! Before each account/provider switch, `snapshot` copies every EXISTING Claude
//! file into a per-name rotating store under `<cchive>/backups/`, then prunes each
...
const KEEP: usize = 20;
/// Subdir under the cchive config dir that holds the rotating backups.
const BACKUPS_SUBDIR: &str = "backups";
/// Suffix marking a cchive backup file: `<name>.<epoch_millis>.bak`.
...
/// Snapshot each EXISTING file in `claude_paths` into `<cchive_dir>/backups/`
...
pub fn snapshot(cchive_dir: &Path, claude_paths: &[PathBuf]) -> Result<(), CoreError> {
    snapshot_at(cchive_dir, claude_paths, atomic_fs::now_millis())
}
...
pub fn list(cchive_dir: &Path) -> Vec<BackupEntry> {
    let backups_dir = cchive_dir.join(BACKUPS_SUBDIR);
...
pub fn restore(cchive_dir: &Path, id: &str) -> Result<(), CoreError> {
    restore_inner(cchive_dir, id, &claude_snapshot_paths())
}

/// Resolve the live cchive config dir + the Claude files a switch mutates and
...
pub fn auto_snapshot() {
    let _ = snapshot(&paths::cchive_config_dir(), &claude_snapshot_paths());
}
...
/// The Claude files cchive backs up before a switch: the live credential, the
...
fn snapshot_at(cchive_dir: &Path, claude_paths: &[PathBuf], millis: u128) -> Result<(), CoreError> {
    let backups_dir = cchive_dir.join(BACKUPS_SUBDIR);
...
fn restore_inner(cchive_dir: &Path, id: &str, originals: &[PathBuf]) -> Result<(), CoreError> {
    let backups_dir = cchive_dir.join(BACKUPS_SUBDIR);
    ...
    let _ = snapshot(cchive_dir, std::slice::from_ref(original));
...
mod tests {
    ...
    fn snapshot_writes_copy_and_prune_keeps_newest_twenty() {
        ...
        let cchive = dir.path().join("cchive");
        ...
        for ts in 1_000..1_022u128 {
            snapshot_at(&cchive, std::slice::from_ref(&original), ts).unwrap();
        }

        let entries = list(&cchive);
        ...
        let newest = cchive.join(BACKUPS_SUBDIR).join(&entries[0].id);
        assert_eq!(fs::read(&newest).unwrap(), b"cred-content");
        #[cfg(unix)]
        {
            assert_eq!(mode_of(&newest), 0o600, "backup file 0600");
            assert_eq!(mode_of(&cchive.join(BACKUPS_SUBDIR)), 0o700, "backups dir 0700");
        }
    }
    ...
    fn list_is_newest_first_across_names() {
        ...
        snapshot_at(&cchive, std::slice::from_ref(&settings), 1_000).unwrap();
        snapshot_at(&cchive, std::slice::from_ref(&cred), 2_000).unwrap();
        snapshot_at(&cchive, std::slice::from_ref(&settings), 3_000).unwrap();

        let timestamps: Vec<i64> = list(&cchive).iter().map(|e| e.timestamp).collect();
    ...
    fn snapshot_skips_missing_files() {
        ...
        snapshot_at(&cchive, &[present.clone(), absent.clone()], 1_000).unwrap();
    ...
    fn restore_copies_back_and_snapshots_current_first() {
        ...
        snapshot_at(&cchive, std::slice::from_ref(&original), 1_000).unwrap();
        let id = list(&cchive)[0].id.clone();
        ...
        restore_inner(&cchive, &id, std::slice::from_ref(&original)).unwrap();
        ...
        let after = list(&cchive);
        assert_eq!(after.len(), 2, "current state snapshotted before restore");
        let newest = cchive.join(BACKUPS_SUBDIR).join(&after[0].id);
    ...
    fn list_missing_backups_dir_is_empty() {
        ...
        assert!(list(&dir.path().join("cchive")).is_empty());
    }
    ...
    fn restore_unknown_id_is_not_found() {
        let dir = tempfile::tempdir().unwrap();
        let cchive = dir.path().join("cchive");
        match restore_inner(&cchive, "settings.json.999.bak", &[]) {
